...
function ajx(target,data)
{
  ...
  $.ajax({
			  ...
			  url: "response.php",
			  data: data,
			  beforeSend:loadingImagen(target),
...
function loadingImagen(target){

/*
<div class="spinner-border text-success" role="status"><span class="sr-only">Loading...</span></div>
*/
	const loading = '<img src="gfx/gif.gif" width="70%">';

    var x=$(target);
	x.html('<center><div class="container"><div class="row"><div class="col-md-12"><div class="lds-roller"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div></div></div><center>');
	//x.html(loading) ;
