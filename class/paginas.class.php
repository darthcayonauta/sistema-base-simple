	...
	public	function navigation($separator = "  ", $css_current = "", $back_forward = false)
	{
		...
		if ($curr_pages <= $all_pages && $curr_pages >= 0) {
			...
			if($all_pages >= 1) {
				...
				$navi_string = ($curr_pages > 0) ? " <li class='pages'><a href='#' class='pagination btn btn-sm btn-light' data=\"".$var."=".$backward.$this::rebuild_qs($var)."\">".$this->str_backward."</a> " : $this->str_backward." ";
				...
				if (!$back_forward) {
					for($a = $start + 1; $a <= $end; $a++){
						...
						if ($theNext != $curr_pages) {
							...
							$navi_string .= " <li class='pages'><a href='#' class='pagination btn btn-sm btn-light' data=\"".$var."=".$theNext.$this::rebuild_qs($var)."\"> ";
			...
			$navi_string .= ($curr_pages < $all_pages) ? " <li class='pages'><a href='#' class='pagination btn btn-sm btn-light' data=\"".$var."=".$forward.$this::rebuild_qs($var)."\">".$this->str_forward."</a></li>" : " <li class='pages'>".$this->str_forward."</li>";
